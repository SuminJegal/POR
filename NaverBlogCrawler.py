...
naver_client_id = "7aGcXGtA2kz8lh6umeDJ"
naver_client_secret = "8WBaBA2Y9L"
...
def naver_blog_crawling(search_blog_keyword, display_count, sort_type):
    ...
    return get_blog_contents(search_blog_keyword, display_count, search_result_blog_page_count, sort_type)
...
def get_blog_contents(search_blog_keyword, display_count, search_result_blog_page_count, sort_type):
    result = ""
    ...
    for i in range(1, search_result_blog_page_count + 1):
        ...
        if response_code is 200:
            ...
            for j in range(0, len(response_body_dict['items'])):
                try:
                    #print(response_body_dict['items'][j])
                    ...
                    for link in get_blog_post_content_soup.select('frame#mainFrame'):
                        ...
                        blog_post_content = get_real_blog_post_content_soup.select("div.__se_component_area,div#postViewArea")[0]
                        blog_post_content_text = blog_post_content.get_text(strip=True)
                    ...
                    '''
                        ...
                        #print("포스팅 내용 : " + blog_post_content_text)
                        result += blog_post_full_contents
                except Exception as e:
                    print(e)
                    j += 1
        print(i,'/',search_result_blog_page_count)
        if i > 10:
            return result
    return result

if __name__ == '__main__':
    contents = naver_blog_crawling("맛집 도곡동", 100, "sim")
